...
type SendManualEmailDialogProps = {
  lead: Lead;
  isOpen: boolean;
  onOpenChange: (isOpen: boolean) => void;
};

export function SendManualEmailDialog({ lead, isOpen, onOpenChange }: SendManualEmailDialogProps) {
  ...
  async function onSubmit(values: z.infer<typeof formSchema>) {
    ...
    try {
      ...
      toast({
        title: 'Email Queued',
        description: `"${selectedTemplate.name}" is being sent to ${lead.email}.`,
      });
      onOpenChange(false);
      form.reset();
    } catch (error: any) {
    ...
    <Dialog open={isOpen} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-xl">
        <DialogHeader>
          ...
          <DialogDescription>
            Send a follow-up email to {lead.contactName} ({lead.email}).
          </DialogDescription>
        </DialogHeader>
        <Form {...form}>
